...
const animation = createSnowflakesAnimation({
  canvas: () => canvas.value ?? undefined,
  snowflakeImage: useSvgImageElement(
    () =>
  ...
  snowflakesCount: 500,
  snowflakesMinDistance: 2,
  snowflakesSize: 24
});

let currentFrame = -1;

watch([canvas, canvasSize.width, canvasSize.height], ([cvs, width, height]) => {
  if (cvs) {
    cvs.width = window.devicePixelRatio * width;
    cvs.height = window.devicePixelRatio * height;
  }
});
